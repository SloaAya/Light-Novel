...
| 📱 手机书源 | 内置 OPDS 1.2 书源服务，公网地址 <https://ranqing.ccwu.cc/>（无需登录即可浏览 / 搜索 / 下载 / 订阅） |
| 🔐 管理员登录 | 网页 `/opds/login` 口令登录（HMAC 签名 cookie，30 天）；阅读器可用 `https://用户:口令@域名/` 走 Basic。未登录 = 访客，只少管理入口 |
| ✅ 已读完清单 | 登录后可逐部标记「已读完」；标记**原地生效不刷新页面** |
| 🔄 已读自动摘除 | 已读完的书一旦补了新卷，自动从「已读完」清单剔除并置顶提示（"读完了"这个结论已经不成立） |
| 🖥️ 控制面板 | Tkinter 面板统一启停与状态查看，可打包为单文件 `LightNovel.exe` |
| 🚀 在线启动器 | 双击 `launchers/launch_online.bat` 一键起「书源 + 隧道 + 守护进程」：关窗口不掉线，服务挂了 30 秒内自动拉起；停止用 `launchers/stop_opds.bat`（严格先停守护再停服务） |
